...
namespace linalg
{
  ...
  Matrix &Matrix::gauss_backward()
  {
    ...
    for (int i = (int)rows() - 1; i >= 0;
         --i)
    { // чтобы i могло стать отрицательным
  ...
  std::istream &operator>>(std::istream &is, Matrix &matrix)
  {
    ...
    try
    {
      ...
      while (!is.eof())
      {
        char border;
        is >> border;
        if (is.eof() && current_row == 0 && data_size == 0)
        {
          break;
        }
        if (border != '|')
        {
          throw std::runtime_error(
              "Invalid format: expected '|' at the start of row " +
        ...
        while (is.peek() != '|' && is.peek() != EOF)
        {
          ...
          if (!(is >> data[data_size++]))
          {
            throw std::runtime_error("Invalid format: could not read number in row " + std::to_string(current_row + 1));
          }
          current_col++;
          is >> std::ws;
        }
        ...
        {
          throw std::runtime_error(
              "Invalid format: unexpected end of stream, missing closing '|' for row " +
              std::to_string(current_row + 1));
        }
        is >> border;
        if (border != '|')
        {
          throw std::runtime_error("Invalid format: expected '|' at the end of row " +
                                   std::to_string(current_row + 1));
        }

        if (current_row == 0)
        {
          first_row_cols = current_col;
        }
        else
        {
          if (current_col != first_row_cols)
          {
            throw std::runtime_error(
                "Invalid format: row " + std::to_string(current_row + 1) +
                " has " + std::to_string(current_col) +
                " elements, but first row has " + std::to_string(first_row_cols));
          }
        }
        if (first_row_cols == 0 && current_row > 0)
        {
          throw std::runtime_error("Invalid format: empty matrix with multiple rows is not allowed, error at row " + std::to_string(current_row + 1));
        }
      ...
      if (data_size > 0)
      {
        if (current_row * first_row_cols != data_size)
        {
          throw std::runtime_error("Invalid format: Inconsistent matrix structure.");
        }
        matrix.reshape(current_row, first_row_cols);
        std::copy(data, data + data_size, matrix.begin());
      }
      else
      {
        matrix.clear();
